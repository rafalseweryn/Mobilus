...
public class AssignGroupEditPlaceFragment extends EditingPlaceFragment {
  ...
  private void populateTableLayout(Place paramPlace) {
    ...
    for (byte b2 = 0; b1 < arrayList.size(); b2++) {
      ...
      appCompatCheckBox.setChecked(group.belongsToPlace(paramPlace.getId()));
      appCompatCheckBox.setTag(paramPlace);
  ...
  protected void saveButtonClicked() {
    ...
    if (place != null) {
      ...
      for (byte b = 0; b < this.groupsCheckBoxes.size(); b++) {
        if (((AppCompatCheckBox)this.groupsCheckBoxes.get(b)).isChecked())
          arrayList.add(Long.valueOf(((Group)((AppCompatCheckBox)this.groupsCheckBoxes.get(b)).getTag()).getId())); 
      } 
      if (arrayList.size() != 0 || (place.getDevices()).length != 0) {
        this.managementServiceBinder.editDevicePlaceGroups(place.getId(), arrayList);
        return;
      } 
...
/* Location:              C:\Users\rafal\Downloads\Mobius_jar.jar!\pl\com\mobilelabs\mobilus\view\fragment\editingplace\AssignGroupEditPlaceFragment.class
