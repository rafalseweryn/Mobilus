...
public class LinearLayoutManager extends RecyclerView.LayoutManager implements ItemTouchHelper.ViewDropHandler, RecyclerView.SmoothScroller.ScrollVectorProvider {
  ...
  private void layoutForPredictiveAnimations(RecyclerView.Recycler paramRecycler, RecyclerView.State paramState, int paramInt1, int paramInt2) {
    ...
    int m = b2;
    for (b2 = b1; b2 < i; b2++) {
      RecyclerView.ViewHolder viewHolder = list.get(b2);
  ...
  public void onLayoutChildren(RecyclerView.Recycler paramRecycler, RecyclerView.State paramState) {
        ...
        if (this.mPendingScrollPositionOffset != Integer.MIN_VALUE) {
          ...
          if (view != null) {
            if (this.mShouldReverseLayout) {
              ...
              k = this.mOrientationHelper.getDecoratedStart(view);
              j = this.mOrientationHelper.getStartAfterPadding();
    ...
    if (getChildCount() > 0)
      if ((this.mShouldReverseLayout ^ this.mStackFromEnd) != 0) {
        i = fixLayoutEndGap(k, paramRecycler, paramState, true);
        m = j + i;
        ...
        k += i;
        n = fixLayoutEndGap(k, paramRecycler, paramState, false);
        m = j + i + n;
        i = k + n;
      }  
    ...
    public void assignFromViewAndKeepVisibleRect(View param1View, int param1Int) {
      ...
      if (this.mLayoutFromEnd) {
        ...
        if (param1Int > 0) {
          ...
          i = this.mOrientationHelper.getEndAfterPadding() - Math.min(0, k - i - n) - j + m;
          if (i < 0)
            this.mCoordinate -= Math.min(param1Int, -i); 
        } 
      } 
  ...
  static class LayoutState {
    ...
    public View nextViewInLimitedList(View param1View) {
      ...
      while (true) {
        view2 = view1;
        if (b < i) {
          view2 = ((RecyclerView.ViewHolder)this.mScrapList.get(b)).itemView;
...
/* Location:              C:\Users\rafal\Downloads\Mobius_jar.jar!\android\support\v7\widget\LinearLayoutManager.class
