...
class FindAddress {
    ...
    while (true) {
      ...
      if (i < paramString.length()) {
        ...
        if (matcher.end() - matcher.start() > 25)
          return -matcher.end(); 
        while (m < matcher.start()) {
          i = k;
          if ("\n\013\f\r  ".indexOf(paramString.charAt(m)) != -1)
            i = k + 1; 
          m++;
          k = i;
        } 
        if (k > 5 || ++i1 > 14)
          break; 
        if (matchHouseNumber(paramString, m) != null) {
          if (n != 0 && k > 1)
            return -m; 
          i = n;
          b1 = b;
          ...
          if (bool) {
            i = j;
            if (i1 > 4) {
              MatchResult matchResult = matchState(paramString, m);
              i = j;
              if (matchResult != null) {
  ...
  private static boolean checkHouseNumber(String paramString) {
    int i = 0;
    int j;
    for (j = i; i < paramString.length(); j = k) {
      int k = j;
...
/* Location:              C:\Users\rafal\Downloads\Mobius_jar.jar!\android\support\v4\tex\\util\FindAddress.class
